...
export function IterationPanel() {
  ...
  return <section className="overtone-whatif">
    ...
    <div className="overtone-row">{['dream', 'dance', 'stripped'].map((key) => <Button key={key} tone="secondary" size="sm"
      disabled={creative.exploring || creative.arranging || creative.musicBusy || !readiness.textCapabilityAvailable}
      onClick={() => void creative.explore(selected, t(`Overtone.playground.reinterpret.${key}.prompt`))}>{t(`Overtone.playground.reinterpret.${key}.name`)}</Button>)}</div>
