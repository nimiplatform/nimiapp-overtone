...
import { SeparationPanel } from './separation-panel.js';
...
export function TakesPanel() {
  ...
  const jobItems: GenerationStatusListProps['items'] = Object.values(state.activeJobs).map(job=>({runId:job.jobId,status:job.status,label:t(job.capability === 'music.transcribe' ? 'Overtone.transcription.title' : job.capability === 'audio.voice.convert' ? 'Overtone.voiceConvert.title' : job.capability === 'audio.separate' ? 'Overtone.separation.title' : 'Overtone.takes.generating')}));
  return <div className="overtone-takes-stack">
    ...
    <ImportRecording /><TranscriptionPanel /><SeparationPanel /><RecoverableResults /><div className="ot-take-list">
  ...
  return <article className="ot-take-row" data-selected={isSelected} data-playing={playing}>
    <div className="ot-take-row__head">
      ...
      <div className="ot-take-info"><Button tone="ghost" className="ot-take-title" aria-pressed={isSelected} onClick={()=>selectTake(take.takeId)}>{take.title}</Button>
        <span>{String(ordinal).padStart(2,'0')}<span> / </span>{formatAudioTime(take.durationSeconds??0)}<span> · </span>{t(take.origin === 'imported-recording' ? 'Overtone.recording.imported' : take.origin === 'local-render' ? 'Overtone.recording.localRender'
          : take.origin === 'runtime-result' && take.capability === 'audio.voice.convert' ? 'Overtone.voiceConvert.shortLabel'
          : take.origin === 'runtime-result' && take.capability === 'audio.separate' ? 'Overtone.separation.shortLabel'
          : take.creationMode === 'song'
    ...
    {take.origin === 'runtime-result' && take.capability === 'music.generate' && take.termination === 'budget-limit' ? <p className="ot-take-parent">{t('Overtone.score.budgetLimit')}</p> : null}
    {take.origin === 'runtime-result' && take.capability === 'audio.separate' ? <p className="ot-take-parent"><OvertoneIcon name="branch" size={12}/>{t('Overtone.separation.stemFacts', { stem: t(`Overtone.separation.stems.${take.separation.stem}`),
      start: (take.separation.sourceRange.startFrame / take.separation.sourceAudio.sampleRateHz).toFixed(3), end: (take.separation.sourceRange.endFrame / take.separation.sourceAudio.sampleRateHz).toFixed(3) })}</p> : null}
    {take.origin === 'runtime-result' && take.capability === 'audio.voice.convert' ? <VoiceConvertRecord take={take} /> : null}
...
function VoiceConvertRecord({take}:{take:SongTake}) {
  const {t}=useTranslation();
  const state=useOvertoneState();
  if (take.origin !== 'runtime-result' || take.capability !== 'audio.voice.convert') return null;
  const derivation=take.derivation;
  const retained=state.project?.takes.find(entry=>entry.takeId===derivation.retainedAccompanimentTakeId);
  const deltaMs=`${derivation.durationDeltaMs>0?'+':''}${derivation.durationDeltaMs}`;
  return <div className="ot-take-parent ot-voice-record" data-testid="voice-convert-record">
    <p><OvertoneIcon name="branch" size={12}/>{t('Overtone.voiceConvert.tailFacts',{relation:t(`Overtone.voiceConvert.lengthRelation.${derivation.lengthRelation}`),deltaMs})}</p>
    <p>{t('Overtone.voiceConvert.mixFacts',{rate:derivation.mixDomain.sampleRateHz,channels:derivation.mixDomain.channels})}</p>
    <p>{t('Overtone.voiceConvert.placementFacts',{start:derivation.mix.vocalStartFrame,rate:derivation.mixDomain.sampleRateHz})}</p>
    <p>{t('Overtone.voiceConvert.retainedFacts',{title:retained?.title??derivation.retainedAccompanimentTakeId})}</p>
    <p>{t(derivation.mix.peak>1?'Overtone.voiceConvert.mixPeakOver':'Overtone.voiceConvert.mixPeak',{peak:derivation.mix.peak.toFixed(3)})}</p>
  </div>;
}
